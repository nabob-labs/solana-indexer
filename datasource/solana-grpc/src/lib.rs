...
async fn send_subscribe_account_update_info(
    ...
    if let Some(account_info) = account_update_info {
        ...
        metrics
            .increment_counter("solana_grpc_account_updates_received", 1)
            .await
...
async fn send_subscribe_update_transaction_info(
    ...
    if let Some(transaction_info) = transaction_info {
        ...
        let Some(solana_transaction) = transaction_info.transaction else {
            return;
        };
        let Some(solana_tx_meta) = transaction_info.meta else {
            return;
        };
        let Ok(versioned_transaction) = create_tx_versioned(solana_transaction) else {
            return;
        };
        let meta_original = match create_tx_meta(solana_tx_meta) {
            Ok(meta) => meta,
            Err(err) => {
        ...
        metrics
            .increment_counter("solana_grpc_transaction_updates_received", 1)
            .await
            .unwrap_or_else(|value| log::error!("Error recording metric: {}", value));
    } else {
        log::error!(
