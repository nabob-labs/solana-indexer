...
use crate::{
    datasource::AccountDeletion, error::IndexerResult, metrics::MetricsCollection,
    processor::Processor,
};
...
/// return a `IndexerResult<()>`, which indicates success or failure.
///
/// # Syntax
///
...
/// # Returns
///
/// Returns a `IndexerResult<()>`, which will be `Ok(())` if processing is successful, or an error
...
pub trait AccountDeletionPipes: Send + Sync {
    ...
    /// # Returns
    ///
    /// Returns a `IndexerResult<()>`, which is `Ok` on success, or an error if processing fails.
