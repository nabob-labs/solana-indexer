...
/// non-blocking operations, and is expected to return a `IndexerResult<()>`,
...
/// use solana_indexer_core::datasource::BlockDetails;
/// use solana_indexer_core::block_details::BlockDetailsPipe;
/// use async_trait::async_trait;
///
...
/// # Returns
///
/// Returns a `IndexerResult<()>`, which will be `Ok(())` if processing is
...
pub trait BlockDetailsPipes: Send + Sync {
    ...
    /// # Returns
    ///
    /// Returns a `IndexerResult<()>`, which is `Ok` on success, or an error if
    /// processing fails.
    async fn run(
        &mut self,
        block_details: BlockDetails,
