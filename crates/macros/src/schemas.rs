...
macro_rules! schema {
    ($($tt:tt)*) => {{
        ...
        solana_indexer_core::schema::TransactionSchema { root: nodes }
    }};
}
...
macro_rules! schema_inner {
    ...
    ($nodes:expr, any $($rest:tt)*) => {
        $nodes.push(solana_indexer_core::schema::SchemaNode::Any);
        schema_inner!($nodes, $($rest)*);
    };

    ($nodes:expr, [$ix_type:expr, $name:expr] $($rest:tt)*) => {
        $nodes.push(solana_indexer_core::schema::SchemaNode::Instruction(solana_indexer_core::schema::InstructionSchemaNode {
            ix_type: $ix_type,
            name: $name.to_string(),
