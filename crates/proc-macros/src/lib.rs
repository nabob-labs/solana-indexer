...
/// `#[derive(IndexerDeserialize)]`. Optionally, use the `#[indexer(discriminator
...
/// Generates an implementation of the `IndexerDeserialize` trait for a given
/// type.
///
...
/// `#[derive(IndexerDeserialize)]`. Optionally, you can specify a
/// `#[indexer(discriminator = "...")]` attribute to define a custom
...
///   appropriate `IndexerDeserialize` implementation.
///
/// # Return
///
/// Returns a `TokenStream` containing the implementation of the
/// `IndexerDeserialize` trait for the given type. If successful, this enables
...
/// containing a `discriminator` key in the format `indexer(discriminator =
...
/// `indexer(discriminator = "...")` attribute is not present, the function
/// returns `None`.
///
/// # Syntax
///
...
/// valid `indexer(discriminator = "...")` attribute is found. If the attribute
