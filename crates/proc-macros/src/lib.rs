//! # Indexer Proc Macros
//!
//! `indexer-proc-macros` is a collection of procedural macros designed to simplify and enhance
//! Rust-based development for Solana programs using the Indexer framework.
...
//! when working with Indexer, particularly around deserialization, instruction
//! decoding, and structuring custom types. By leveraging `indexer-proc-macros`, you can reduce
//! the amount of manual coding and ensure consistent, performant handling of Solana-specific data
//! structures.
//!
//! ## Key Features
//!
//! - **`IndexerDeserialize`**: Automatically implement the `IndexerDeserialize` trait for
...
//! - The macros provided are optimized for use within the Indexer framework.
//!
//! ## Contribution
//!
//! Contributions are welcome! If you have ideas for improving or expanding the functionality
//! of `indexer_macros`, please consider submitting a pull request or opening an issue on the
...
/// Automatically generates an implementation of the `IndexerDeserialize` trait.
///
/// This derive macro creates the `IndexerDeserialize` implementation for a given
...
/// `#[indexer(discriminator)]` attribute, the macro uses this field's value as a
/// discriminator to match and validate data during deserialization.
///
/// # Syntax
///
/// To use this macro, annotate your struct or enum with `#[derive(IndexerDeserialize)]`.
/// Optionally, use the `#[indexer(discriminator = "0x...")]` attribute to specify a
...
///   corresponding `IndexerDeserialize` implementation.
///
/// # Return
///
/// Returns a `TokenStream` representing the generated `IndexerDeserialize`
...
/// # Notes
///
/// - The `#[indexer(discriminator = "0x...")]` attribute is optional. If not
...
///
#[proc_macro_derive(IndexerDeserialize, attributes(indexer))]
pub fn indexer_deserialize_derive(input_token_stream: TokenStream) -> TokenStream {
    ...
    let expanded = quote! {
        ...
        impl solana_indexer_core::deserialize::IndexerDeserialize for #name {
...
/// Generates an implementation of the `IndexerDeserialize` trait for a given type.
///
/// This procedural macro automatically derives the `IndexerDeserialize` trait
...
/// and allows for customized deserialization using the `#[indexer]` attribute to
/// specify a unique discriminator for the type.
///
/// # Syntax
///
/// To use this macro, annotate the target type with `#[derive(IndexerDeserialize)]`.
/// Optionally, you can specify a `#[indexer(discriminator = "...")]` attribute to
...
///   `IndexerDeserialize` implementation.
///
/// # Return
///
/// Returns a `TokenStream` containing the implementation of the `IndexerDeserialize`
...
/// these are the only supported forms for `IndexerDeserialize` derivation.
/// Additionally, an invalid or missing `#[indexer]` attribute may result in a
...
///
/// This function searches through a list of attributes for a `indexer` attribute
/// containing a `discriminator` key in the format `indexer(discriminator = "0x...")`.
/// If found, it parses the discriminator as a hexadecimal string and returns it
/// as a byte slice within a `TokenStream`. If the `indexer(discriminator = "...")`
...
/// valid `indexer(discriminator = "...")` attribute is found. If the attribute is
/// not present, or if the value is not a valid hexadecimal string, the function
/// returns `None`.
///
/// # Errors
///
/// If the `indexer(discriminator = "...")` attribute contains an invalid hex
...
pub fn instruction_decoder_collection(input: TokenStream) -> TokenStream {
    ...
    for entry in entries {
        ...
        parse_instruction_arms.push(quote! {
            if let Some(decoded_instruction) = #decoder_expr.decode_instruction(&instruction) {
                return Some(solana_indexer_core::instruction::DecodedInstruction {
    ...
    let expanded = quote! {
        ...
        impl solana_indexer_core::collection::InstructionDecoderCollection for #instructions_enum_name {
